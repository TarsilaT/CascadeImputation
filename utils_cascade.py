...
#### Attribute ordering criteria #### 
def attribute_order_criterion_only(df_mask, df_correl, order_column="noSort"): 
    """
    Determines the order of attributes according to the selected criteria
...
def cascade_imputation(df_filled, df_null, df_mask, ordered_clu_list, ordered_col_list, knn_neighbors, knn_metric='euclidean'): # knn_neighbors=3
    """
...
#### Cascade imputation (G2 group) #### 
def cascade_only_imputation(df_filled, df_null, ordered_col_list, knn_neighbors, knn_metric='euclidean'): # knn_neighbors=3
    """
    Perform cascade imputation=
    ...
    for attribute in ordered_col_list:            
        #Step 1: Create cluster dataframe by complete and incomplete subset (based on attribute)
        start_time1 = time.time()                                  # Inicializar a contagem do tempo etapa 1
        df_cluster, df_attr_null = get_cluster_df(df_filled, df_null, df_restorado, attribute)
        ...
        # Ensure that the number of neighbors does not exceed the number of samples
        #effective_k = min(knn_neighbors, n_samples) if n_samples > 0 else 0

        #if effective_k > 0:
        #    nn_model = NearestNeighbors(n_neighbors=effective_k, metric=knn_metric)
        #    nn_model.fit(aux_knn)
        #    distances_, indices = nn_model.kneighbors(aux_knn)
        #else:
        #    distances_, indices = [], []
...
def error_metrics(df_original, df_imputado):
    # Initialize an empty DataFrame to store the averages
    mse_values_df  = pd.DataFrame(columns=df_original.columns)
    rmse_values_df = pd.DataFrame(columns=df_original.columns)
    mae_values_df  = pd.DataFrame(columns=df_original.columns)
    mape_values_df = pd.DataFrame(columns=df_original.columns)
    ...
    for column in df_original.columns:
        ...
        mse = reg_metric(y_true, y_pred, type="mse")
        mse_values_df[column] = [mse]
        ...
        # Calculate RMSE for each column
        mae = reg_metric(y_true, y_pred, type="mae")
        mae_values_df[column] = [mae]

        # Calculate RMSE for each column
        mape = reg_metric(y_true, y_pred, type="mape")
        mape_values_df[column] = [mape]
    ...
    avg_mse = mse_values_df.mean(axis=1).values[0]              # Calculate MSE
    avg_rmse = rmse_values_df.mean(axis=1).values[0]            # Calculate RMSE
    avg_mae = mae_values_df.mean(axis=1).values[0]              # Calculate MAE
    avg_mape = mape_values_df.mean(axis=1).values[0]            # Calculate MAPE
    ...
    return avg_mse, avg_rmse, avg_mae, avg_mape, avg_sim_error, correl_bias
...
def eraser_with_params(df, params):
    ...
    p_miss = float(params['pct_ausencia'])    # % valores ausentes
...
def cascade_imputation_with_params(df_incompleto, params):
    ...
    logging.info(f"----------------")
    logging.info(f"Inicio da imputação cascata completa da simulação {idx_simulacao}...")
...
# Função para realizar a IMPUTAÇÃO CASCATA com os parâmetros fornecidos
def cascade_only_imputation_with_params(df_incompleto, params):
    ## Extrair os parâmetros de cada simulação
    idx_simulacao = params['idx_simulacao']
    order_column = params['order_column']
    n_clusters_knn = params['n_clusters_knn']  
    ...
    # Attribute ordering criterion
    ordered_col_list = attribute_order_criterion_only(df_mask, df_correl, order_column=order_column)
            
    # Cascade imputation
    df_cascade_imp = cascade_only_imputation(df_filled, df_null, ordered_col_list, knn_neighbors=n_clusters_knn, knn_metric='euclidean')
